...
export const SettingsModal = ({ open = true }: SettingsModalProps) => {
  ...
  return (
    <AnimatePresence mode="wait">
      {open && (
        <motion.div
          className="z-30 absolute top-0 left-0 w-full h-dvh bg-black/20 shadow-modal flex items-center justify-center"
          initial={{ opacity: 0 }}
          animate={{ opacity: 100 }}
          exit={{ opacity: 0 }}
          ...
          onClick={close}
        >
          {/** biome-ignore lint/a11y/noStaticElementInteractions: modal behavior */}
          {/** biome-ignore lint/a11y/useKeyWithClickEvents: modal behavior */}
          <div
            className="rounded-card bg-white py-7 px-7.5 w-110 flex flex-col gap-3"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-between">
              <div className="flex items-center gap-3">
                <Avatar size="lg" username="art14" />
            ...
            <LabelInput label="Email" id="email" />
